...
(function( $, window, document, undefined ) {
	var Slider = {
		...
		build: function() {
			...
			// Add the .panelcoca class to the individual panelcocas (backwards compatable)
			$(self.sliderId + " > div").addClass('panelcoca');
			self.panelcocaClass = self.sliderId + ' .panelcoca';
			// Wrap all panelcocas in a div, and wrap inner content in a div (backwards compatible)
			$(self.panelcocaClass).wrapAll('<div class="panelcoca-container"></div>');
			if ( $(self.panelcocaClass).children().attr('class') != 'panelcoca-wrapper' ) { $(self.panelcocaClass).wrapInner('<div class="panelcoca-wrapper"></div>'); }
			self.panelcocaContainer = ($(self.panelcocaClass).parent());
...
//			self.slideWidth = $(self.sliderId).width();
		...
		addNavigation: function(){
			...
			// Add labels
			$.each(
				(self.$elem).find(self.options.panelcocaTitleSelector), function(n) {
		...
		setCurrent: function( direction ){
			...
			if (self.clickable) {
				...
				if (typeof direction == 'number') {	self.currentTab = direction;	}
					...
					// If not continuous, slide back at the last or first panelcoca
					if (!self.options.continuous){
						self.currentTab = (self.currentTab < 0) ? this.panelcocaCount - 1 : (self.currentTab % this.panelcocaCount);
					}
				}
				// This is so the height will match the current panelcoca, ignoring the clones.
				// It also adjusts the count for the "currrent" class that's applied
				if (self.options.continuous) {
					self.panelcocaHeightCount = self.currentTab + 1;
					if (self.currentTab === self.panelcocaCount - 2){self.setTab = 0;}
					else if (self.currentTab === -1) {self.setTab = self.panelcocaCount - 3;}
					else {self.setTab = self.currentTab;}
				}
				else{
					self.panelcocaHeightCount = self.currentTab;
				...
				if (self.options.continuous) {
					if (self.currentTab === self.panelcocaCount - 2) {
						window.location.hash = 1;
					} else if (self.currentTab === -1) {
						window.location.hash = self.panelcocaCount - 2;
					} else {
		...
		transition: function(){
			...
			if (self.options.autoHeight) {
				$(self.panelcocaContainer).parent().animate({
					'height': $($(self.panelcocaContainer).children()[self.panelcocaHeightCount]).height()
				}, {
			...
			// Animate the slider
			(self.panelcocaContainer).animate({
		...
		continuousSlide: function (delay){
			...
			if (self.options.continuous) {
				self.continuousTimeout = setTimeout(function() {
					...
					// If on the last panelcoca (the clone of panelcoca 1), set the margin to the original.
					if (self.currentTab === self.panelcocaCount - 2){
						$(self.panelcocaContainer).css('margin-left', -self.slideWidth + self.pSign);
						self.currentTab = 0;
					}
					// If on the first panelcoca the clone of the last panelcoca), set the margin to the original.
					else if (self.currentTab === -1){
						$(self.panelcocaContainer).css('margin-left', -( ((self.slideWidth * self.panelcocaCount) - (self.slideWidth * 2))) + self.pSign );
						self.currentTab = (self.panelcocaCount - 3);
					}
					self.clickable = true;
				}, delay);
			}
	...
	$.fn.codaSlider.options = {
		...
		firstPanelToLoad: 1,
		hashLinking: false,
		panelcocaTitleSelector: "h2.title",
